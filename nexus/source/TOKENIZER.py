...
class TOKENIZER:
    ...
    def lexate(code_lines):
        ...
        if len(buffer) > 0:
            ERROR.throw_error(f'(ErroDeToken) Texto não fechado {buffer}.')
    ...
    def set_tokens(lexed_text):
        ...
        for i in range(len(lexed_text)):
            ...
            if not (each.isnumeric() or each == '.') and len(open_num) > 0:
                if float_p == 0:
                    p_nodes.append(NUM(T_INTEIRO, open_num))
                elif float_p == 1:
                    if len(open_num) == 2:
                        open_num += '0'
                    p_nodes.append(NUM(T_V, open_num))
                else:
                    ERROR.throw_error(f'(ErroDesintaxe) Múltiplos números com vírgula em um número {open_num}.')
                float_p = 0
                open_num = ''
            ...
            if each in specialChars:
            ...
            elif each == 'verdadeiro' or each == 'falso':
                ...
                if each.isalnum():
                    ...
                    ERROR.throw_error(f'(ErroDeSintaxe) O operador "{each}" não existe.')
